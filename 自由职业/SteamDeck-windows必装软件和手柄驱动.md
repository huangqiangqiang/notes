# Steam Deck 安装 windows 后必装软件和手柄驱动（保姆级教学）
...
首先，我们要设置开机时的自动登录，打开[AutoLogin](https://learn.microsoft.com/en-us/sysinternals/downloads/autologon) 页面下载脚本，打开 `Autologon.exe`，输入密码，点击 enable。

> 注意：这里输入密码是不会进行校验正确性的。

然后再设置睡眠唤醒不询问密码，`设置 - 搜索 ”登录“ - 打开 ”登录选项“ - 需要登录改为“从不”`

### 打开触摸键盘
很多情况下我们需要触屏输入，这时会发现没有弹出键盘。我们可以在 设置 - 设备 - 输入 - 开启 “不处于平板电脑模式且为连接键盘时显示触摸键盘”
...
我们下载一个叫 incontrol 的软件，地址：https://www.softpedia.com/get/Tweak/System-Tweak/Gibson-InControl.shtml
打开 exe，点击 take control，就表示禁用更新，再次点击 release control，就表示启用更新。
