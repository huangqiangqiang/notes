...
```
mysqldump -p -A -F --single-transaction --master-data=2 {your_database} > /test.sql
```

然后把 `test.sql` 导入到 slave 数据库。
...
如果执行过上面的 test.sql 导入 slave 的步骤，binlog 信息可以从 sql 中获取：
